 ...
 * SoundCloud's API, interpreting them into appropriate exceptions.
 * 
...
class SoundCloudErrorHandler extends DefaultResponseErrorHandler {
	...
	/**
	 * Examines the error data returned from SoundCloud and throws the most
	 * applicable exception.
	 * 
	 * @param errorDetails
	 *            a Map containing an "error_message"
	 */
	...
	private List<Map<String, String>> extractErrorDetailsFromResponse(ClientHttpResponse response) throws IOException {
		...
		if (authenticateHeader != null) {
			json = "{"
					+ authenticateHeader.replace('=', ':').replace("OAuth realm", "\"OAuth realm\"")
							.replace("error", "\"error\"") + "}";
			try {
